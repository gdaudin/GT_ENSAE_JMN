...
\author[rvt]{Guillaume Corlay}
\author[rvt]{Stephane Dupraz\corref{cor1} \fnref{fn1}}
\author[rvt]{Claire Labonne}
\author[rvt]{Anne Muller}
\author[rvt]{Guillaume Daudin}
\cortext[cor1]{Corresponding author}
\ead{guillaume.daudin@dauphine.fr} \fntext[fn1]{Present
affiliation: Columbia University. Personal address : 535 W.
111th street, apt 64, 10025 New York NY. Phone number:
646-320-9026.}

\address[rvt]{ENSAE, French National School of Statistics and Economic Administration, 3, avenue Pierre Larousse, 92245 Malakoff Cedex, FRANCE}
...
\begin{keyword}
Trade costs, globalization, gravity model, aggregation,
structure effect.
\end{keyword}
...
\cite{JMN2011} suggest\footnote{The method is also developed in
\cite{JMN2008}, \cite{JMN2010}, \cite{NOVY}.} a method for
inferring international trade costs from trade flows. They
define trade costs as all barriers to trade, notably transport
and transaction costs. Using the general equilibrium model of
\cite{AW2003}, the authors calculate those costs and their
...
trade costs and economic expansion. Their main result concerns
...
We argue that this result is actually driven by the ad hoc
...
away. We show that the authors' main results is not robust to
the aggregation technique because the one they use also
captures structure effects. The inferred difference between the
two globalization concerns the size of the countries comes from
the phenomenon of globalization, not trade costs. Moreover, we
cast doubt on the possibility to reach conclusions of this
nature through an approach that relies on the study of trade
flows only. Deducing trade costs from trade flows, and using
...
gravity regressions. It then highlights how paradoxical the
...
of the demand-side model.} and each country is initially
...
circularity intuition is wrong. Decomposing the level of
...
We are grateful to Celine Antonin (OFCE) and Guillaume Daudin
(EQUIPPE (Lille I) \& OFCE) for their constant supply of
helpful comments. They, of course, bear no responsibility for
remaining errors.
