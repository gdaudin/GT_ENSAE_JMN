...
The authors use a weighted arithmetic average of their measure of bilateral trade costs to compute country-specific trade costs. Putting aside the weight, this is equivalent to using a power mean with exponent $1/(1-\sigma)$ of the values of dyadic trade flows ($\sigma > 1$ is the elasticity of substitution).
...
However, Jacks, Meissner and Novy also suggest in their article that \cite{AW2003}'s model can be used to move away from the gravity regression approach. Instead of explaining trade flows by observable proxies for costs, they quantify the impact of all impediments to trade from trade flows:
...
Yet, Jacks \emph{et al.}'s results seem to show otherwise. When we decompose the level of exportations of a country simply as the product of its GDP and its openness ratio (see table \ref{OR}) we find that for both globalizations the increase in exportations is mainly explained by GDP growth for the large majority of the 27 countries in the data.\footnote{We follow the authors in interpreting log differences as percentages. However, one should keep in mind that given the size of the changes, this is a very inexact approximation.}
...
As the next section shows, this difference is actually driven by the \textit{ad hoc} way they aggregate the measure of bilateral trade costs.
...
%The last sentence ""The drawback of..." seemed peripheral and a distraction from the main line of thought? Besides, it is not seem more data-intensive than Jacks et al's measure
...
The reason why Jacks \textit{et al.}'s aggregate measure of trade costs yields different conclusions compared to the micro-founded method (and openness rates) is its sensitivity to structure
...
to be equal. Yet it is essential to highlight that
